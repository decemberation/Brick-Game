...
void Paddle::Draw(int temp)
{
	...
	if (temp == 1) {
		setcolor(YELLOW); //Khởi tạo màu đỏ
		rectangle(left, top, right, bottom); // Vẽ hình chữ nhật rỗng theo toạ độ hiện tại
		setfillstyle(1, YELLOW); //Lấy kiểu tô nền
		floodfill(left + 5, bottom - 5, YELLOW); //Tô nền hình chữ nhật
	}
	if (temp == 2) {
		setcolor(WHITE); //Khởi tạo màu đỏ
		rectangle(left, top, right, bottom); // Vẽ hình chữ nhật rỗng theo toạ độ hiện tại
		setfillstyle(1, WHITE); //Lấy kiểu tô nền
		floodfill(left + 5, bottom - 5, WHITE); //Tô nền hình chữ nhật
	}
}
...
{
	if (left+x >= 0 && left+x<=WIDTH-200)//Không cho ván vượt quá 2 biên dọc
	{
