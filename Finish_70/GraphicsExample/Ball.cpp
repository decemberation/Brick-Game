...
void Ball::Draw(int a)
{
	...
	if (a == 1) {
		setcolor(RED); // Thiết lập màu sắc trái banh
		circle(x, y, 5); // Vẽ hình tròn bán kính bằng 5
		setfillstyle(1, RED); // Chọn mẫu tô và màu tô
		floodfill(x, y, RED); // Tô màu hình tròn vừa vẽ
	}
	if (a == 2) {
		setcolor(YELLOW); // Thiết lập màu sắc trái banh
		circle(x, y, 5); // Vẽ hình tròn bán kính bằng 5
		setfillstyle(1, YELLOW); // Chọn mẫu tô và màu tô
		floodfill(x, y, YELLOW); // Tô màu hình tròn vừa vẽ
	}
}
	...
	else
	{
		if (a.isTop() == 2) // Kiểm tra có phải thanh trượt dưới không
		{
			if ((y + 5) >= a.Gettop() - 5 - speedY && (x - 5) >= a.Getleft() - 10 && (x + 5) <= a.Getright() + 10) // Kiếm tra có chạm thanh trượt
			{
				...
				speedX = 6 * sin(Angle(a)); // Đổi hướng banh
				speedY *= -1;
			}
			else
			{
...
void Ball::checkPaddlePvC(Paddle a, int& color)
{
	if (a.isTop() == 1) // Kiếm tra có phải thanh trượt trên cùng
	{
		if ((y - 5) <= a.Getbottom() + 5 - speedY && (x - 5) >= a.Getleft() - 10 && (x + 5) <= a.Getright() + 10) // kiểm tra có chạm vào thanh trượt
...
void Ball::setSpeedX(float a)
{
	this->speedX = a;
}
void Ball::setSpeedY(float a)
{
	this->speedY = a;
}
void Ball::setX(float a)
{
	this->x = a;
}
void Ball::setY(float a)
{
	this->y = a;
}
...
void Ball::setScore1(int a)
{
	this->Score1 = a;
}
void Ball::setScore2(int a)
{
	this->Score2 = a;
}
Ball::Ball()
